...
Example: If required words are "cat, run, big" you might write "The big cat can run fast."`;
  ...
  // Initialize or update an anonymous session (no auth required)
  app.post("/api/analytics/session", async (req, res) => {
    try {
      ...
      const session = await storage.getOrCreateAnonymousSession(
        sessionId, 
        userAgent, 
        referrer as string
      );
  ...
  // Track an analytics event (no auth required)
  app.post("/api/analytics/event", async (req, res) => {
    try {
      ...
      await storage.getOrCreateAnonymousSession(sessionId);
      
      const event = await storage.trackEvent(sessionId, eventType, eventData, userId);
      res.json(event);
    } catch (error) {
  ...
  // Link an anonymous session to a user account (called after signup)
  app.post("/api/analytics/session/link", ensureAuthenticated, async (req, res) => {
    try {
      const userId = getUserId(req);
      const { sessionId } = req.body;
      ...
      // Track the signup completion event
      await storage.trackEvent(sessionId, "signup_completed", {}, userId);
  ...
  // Get guest progress (no auth required)
  app.get("/api/analytics/guest-progress/:sessionId", async (req, res) => {
    try {
      const { sessionId } = req.params;
      const progress = await storage.getGuestProgress(sessionId);
      res.json(progress || null);
    } catch (error) {
      console.error("Error getting guest progress:", error);
      res.status(500).json({ error: "Failed to get guest progress" });
    }
  });

  // Update guest progress (no auth required)
  app.patch("/api/analytics/guest-progress/:sessionId", async (req, res) => {
    try {
      const { sessionId } = req.params;
      const data = req.body;
      ...
      const progress = await storage.updateGuestProgress(sessionId, data);
      res.json(progress);
    } catch (error) {
      console.error("Error updating guest progress:", error);
      res.status(500).json({ error: "Failed to update guest progress" });
    }
  });

  // Migrate guest progress to a child account (called after creating a child)
  app.post("/api/analytics/migrate-progress", ensureAuthenticated, async (req, res) => {
    try {
      const { sessionId, childId } = req.body;
      const userId = getUserId(req);
      
      if (!sessionId || !childId) {
        return res.status(400).json({ error: "Session ID and child ID are required" });
      }
      ...
      await storage.migrateGuestProgressToChild(sessionId, childId);
  ...
  // Get session funnel metrics (admin only - for now just auth required)
  app.get("/api/analytics/funnel", ensureAuthenticated, async (req, res) => {
    try {
      const metrics = await storage.getSessionFunnelMetrics();
      res.json(metrics);
    } catch (error) {
